...
?>
    ...
    public function whatsapp_page_callback()
    {
        ...
        $has_preview_target = $has_agents || !empty($phone);
    ...
    ?>
                    ...
                    <div id="wa-tab-style" class="wa-tab-content" style="display:<?php echo $wa_active_tab === 'style' ? 'block' : 'none'; ?>;">
                        <div class="sad-card sad-mb-16">
                            ...
                            <table class="form-table">
                                <tr>
                                    ...
                                    <td>
                                        <select id="sweetaddons_whatsapp_bubble_style" name="sweetaddons_whatsapp_bubble_style">
                                            ...
                                            <option value="split" <?php selected($bubble_style, 'split'); ?>>Split (Pill + Circle Icon)</option>
                                        </select>
                                        <p class="description">Choose between circle icon, extended button, or split pill with separate icon.</p>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">
                                ...
                                <tr>
                                    <th scope="row">
                                        <label for="sweetaddons_whatsapp_split_text_color">Warna Pill Text</label>
                                    </th>
                                    <td>
                                        <div style="display:flex; align-items:center; gap:8px;">
                                            <input type="color" id="sweetaddons_whatsapp_split_text_color" name="sweetaddons_whatsapp_split_text_color" value="<?php echo esc_attr($split_text_color); ?>" />
                                            <input type="text" value="<?php echo esc_attr($split_text_color); ?>" class="regular-text" readonly style="width:auto;" />
                                        </div>
                                        <p class="description">Warna teks untuk pill kiri pada style Split.</p>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">
                        ...
                        <div class="sad-card">
                            ...
                            <div id="whatsapp-preview-stage" style="position: relative; height: 200px; background: #f9f9f9; border: 2px dashed #ddd; border-radius: 8px; overflow: hidden;">
                                <div id="whatsapp-preview-container" style="display: <?php echo ($enable && $has_preview_target) ? 'block' : 'none'; ?>;">
                                    <div id="whatsapp-preview-bubble" class="sweetaddons-wa-widget sweetaddons-wa-preview sweetaddons-wa-<?php echo esc_attr($position); ?>" data-animation="<?php echo esc_attr($animation); ?>" style="position: absolute; --sweetaddons-wa-split-text-bg: <?php echo esc_attr($split_text_bg); ?>; <?php echo ($position === 'bottom-right') ? 'bottom: 20px; right: 20px;' : 'bottom: 20px; left: 20px;'; ?>">
                                        <div id="whatsapp-preview-shell" class="sweetaddons-wa-bubble sweetaddons-wa-<?php echo esc_attr($bubble_style); ?>">
                                            <div id="whatsapp-preview-inner" class="sweetaddons-wa-link" style="<?php echo ($bubble_style === 'extended') ? 'display:flex; align-items:center; padding:12px 20px; background:' . esc_attr($color) . '; border-radius:25px; color:white; text-decoration:none;' : (($bubble_style === 'split') ? 'display:inline-flex; flex-direction:row-reverse; align-items:center; gap:8px; padding:0; background:transparent; border-radius:999px; color:#111; text-decoration:none; box-shadow:none;' : 'width:60px; height:60px; display:flex; align-items:center; justify-content:center; background:' . esc_attr($color) . '; border-radius:50%; color:white; text-decoration:none;'); ?>">
                                                ...
                                                <span id="whatsapp-preview-text" class="sweetaddons-wa-text" style="font-size: 14px; font-weight: 500; display: <?php echo ($bubble_style === 'extended' || $bubble_style === 'split') ? 'inline-flex' : 'none'; ?>;<?php echo ($bubble_style === 'split') ? ' position:relative; align-items:center; padding:12px 18px 12px 20px; background:' . esc_attr($split_text_bg) . '; color:' . esc_attr($split_text_color) . '; border-radius:999px;' : ''; ?>"><?php echo esc_html($button_text); ?></span>
                                            </div>
                                ...
                                <div id="whatsapp-preview-placeholder" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; color: #666; display: <?php echo ($enable && $has_preview_target) ? 'none' : 'block'; ?>;">
                                    <p>Aktifkan WhatsApp dan isi nomor atau agen untuk melihat preview</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                // Color picker sync
                $('#sweetaddons_whatsapp_color, #sweetaddons_whatsapp_split_text_bg, #sweetaddons_whatsapp_split_text_color').on('change', function() {
                ...
                function updateWhatsAppPreview() {
                    ...
                    const has_fallback_phone = <?php echo !empty($phone) ? 'true' : 'false'; ?>;
                    const has_preview_target = has_agents || has_fallback_phone;
                    ...
                    const split_text_bg = $('#sweetaddons_whatsapp_split_text_bg').val();
                    const split_text_color = $('#sweetaddons_whatsapp_split_text_color').val();
                    ...
                    if (enable && has_preview_target) {
                        ...
                        let innerStyle = '';
                        let iconWrapStyle = '';
                        let textStyle = 'font-size: 14px; font-weight: 500;';
                        $shell.removeClass('sweetaddons-wa-circle sweetaddons-wa-extended sweetaddons-wa-split').addClass(`sweetaddons-wa-${bubble_style}`);
                        if (bubble_style === 'extended') {
                            innerStyle = `display: flex; align-items: center; padding: 12px 20px; background: ${color}; border-radius: 25px; color: white; text-decoration: none;`;
                            $text.show().text(button_text);
                            $icon.css('margin-right', '8px');
                        } else if (bubble_style === 'split') {
                            innerStyle = 'display:inline-flex; flex-direction:row-reverse; align-items:center; gap:8px; padding:0; background:transparent; border-radius:999px; color:#111; text-decoration:none; box-shadow:none;';
                            iconWrapStyle = `display:flex; align-items:center; justify-content:center; width:44px; height:44px; border-radius:50%; background:${color}; color:#fff; flex:0 0 44px;`;
                            textStyle += ` display:inline-flex; align-items:center; padding:12px 18px 12px 20px; background:${split_text_bg}; color:${split_text_color}; border-radius:999px;`;
                            $text.show().text(button_text);
                            $icon.css('margin-right', '0');
                        } else {
                            innerStyle = `width: ${size}px; height: ${size}px; display: flex; align-items: center; justify-content: center; background: ${color}; border-radius: 50%; color: white; text-decoration: none;`;
                            $text.hide();
                            $icon.css('margin-right', '0');
                        }
                        $inner.attr('style', innerStyle);
                        $bubble.find('.sweetaddons-wa-icon').attr('style', iconWrapStyle);
                        $text.attr('style', textStyle + ($text.is(':visible') ? '' : ' display: none;'));
                ...
                $('#sweetaddons_whatsapp_split_text_bg').on('change', updateWhatsAppPreview);
                $('#sweetaddons_whatsapp_split_text_color').on('change input', updateWhatsAppPreview);
